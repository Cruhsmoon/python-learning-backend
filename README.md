...
pytest --cov=fastapi_app --cov-branch --cov-fail-under=80
...
└── unit/
    ...
    └── test_day4_functions.py   # 42 edge-case unit tests
```
...
| **`patch_production_engine` (autouse, session)** | Replaces `fastapi_app.main.engine` with the SQLite test engine once per session so that any code path that reads the module-level `engine` (including the lifespan handler) uses SQLite instead of PostgreSQL. |
...
| **`lifespan` handler instead of module-level `create_all`** | Moving `Base.metadata.create_all` from module scope into the FastAPI lifespan handler means importing `fastapi_app.main` no longer requires a live database. The handler reads the module-level `engine` at call time, so the test patch takes effect before the first call. |
